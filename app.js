...
import { getFirestore, collection, addDoc, getDocs, updateDoc, deleteDoc, doc, query, orderBy, serverTimestamp, setDoc, getDoc } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
async function loadProducts() {
    ...
    try {
        const q = query(collection(db, "produtos"), orderBy("dataDeCriacao", "desc"));
...
async function loadAppearance() {
    try {
        const docRef = doc(db, "config", "appearance");
...
async function saveAppearance() {
    ...
    try {
        ...
        await setDoc(doc(db, "config", "appearance"), config);
    ...
    try {
        if (active) {
            ...
            for (const n of notifications) {
                if (n.id !== id && n.active) {
                    await updateDoc(doc(db, "notifications", n.id), { active: false });
                }
            }
        }
...
async function handleProductSubmit(e) {
    ...
    try {
        ...
        if (editingProductId) {
            await updateDoc(doc(db, "produtos", editingProductId), productData);
